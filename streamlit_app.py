...
def build_flat(df: pd.DataFrame, col_map: Dict[str, Any]) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        prio = r.get(col_map['priority'], "") if pd.notna(r.get(col_map['priority'])) else ""
        summ = r.get(col_map['sum'], "") if pd.notna(r.get(col_map['sum'])) else ""
        desc = r.get(col_map['desc'], "") if pd.notna(r.get(col_map['desc'])) else ""
        over_exp = r.get(col_map['overall_expected'], "") if pd.notna(r.get(col_map['overall_expected'])) else ""
        ...
        base_info = {
            ...
            "Summary": summ,
            "Description": desc,
            "Overall Expected Result": over_exp,
...
st.caption("CSV (noktalı virgül ; ile ayrılmış) → Ayrıştırılmış step tablosu + Ayrı Sütunlar (Tam Jira Uyumu)")
...
with st.expander("Sütun eşlemesi (katı eşleşme ile algılandı)"):
    st.write({
        ...
        "Overall Expected Result": col_map['overall_expected'] or "(Bulunamadı)",
